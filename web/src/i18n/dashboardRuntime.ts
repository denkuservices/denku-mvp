...
export function translateDashboardCopy(
  ...
  const rules: Array<string | null> = [
    ...
    replaceMatch(source, /^Back to (.+)$/, {
      ...
      de: (label) => `Zurück zu ${dictionary[label] ?? label}`,
      // "X adımına dön" reads well after a noun ("İşletmeniz") and badly after a clause
      // ("Neyi yanıtlıyor"), and two of the six step labels are clauses. "Geri: X" carries the
      // same meaning and takes any label.
