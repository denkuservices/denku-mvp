 ...
 *
 * Colour follows the Solana family the owner asked for — a single diagonal
 * violet → cyan → green gradient spanning the WHOLE mark rather than per blade,
 * so the three blades read as one swept object instead of three stickers.
 *
 * ⚠️ Palette note: the site system is teal-black + copper and doc 17 explicitly
 * bans violet, so this mark is deliberately louder than its surroundings. The
 * stops live in `MARK_STOPS` below — retinting to the site palette is a one-line
 * change if the contrast ever reads as imported rather than intentional.
 */
...
/** Solana-family gradient. Change these three to retint the entire brand. */
export const MARK_STOPS = ["#9945FF", "#00D1FF", "#14F195"] as const;
      ...
      {variant === "gradient" && (
        <defs>
          ...
          <linearGradient id={id} x1="10" y1="9" x2="54" y2="55" gradientUnits="userSpaceOnUse">
            <stop offset="0.02" stopColor={MARK_STOPS[0]} />
            <stop offset="0.4" stopColor={MARK_STOPS[1]} />
