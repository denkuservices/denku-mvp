'use client';

/* eslint-disable */

import { HiX } from 'react-icons/hi';
import SidebarLinks from './Links';
import { NavRoute } from './types';

/**
 * Sidebar adapter for Horizon shell.
 * Mirrors Horizon's Sidebar structure but uses our custom Links component
 * that supports '/dashboard' layout routes.
 */
function SidebarAdapter(props: { routes: NavRoute[]; [x: string]: any }) {
  const { routes, open, setOpen } = props;
  return (
    <>
      {/* Overlay - only on mobile when open */}
      {open && (
        <div
          className="fixed inset-0 z-[998] bg-black/40 xl:hidden"
          onClick={() => setOpen(false)}
        />
      )}
      <div
        className={`sm:none duration-175 linear fixed left-0 top-0 h-screen w-[280px] !z-[999] flex min-h-full flex-col bg-white pb-10 shadow-2xl shadow-white/5 transition-all dark:!bg-navy-800 dark:text-white xl:!z-0 ${
          open ? 'translate-x-0' : '-translate-x-96 xl:translate-x-0'
        }`}
      >
        <span
          className="absolute right-4 top-4 block cursor-pointer xl:hidden"
          onClick={() => setOpen(false)}
        >
          <HiX />
        </span>

        {/* Desktop: keep original spacing, mobile: no top padding (drawer is on top) */}
        <div className={`mx-[56px] mt-[50px] flex items-center`}>
          <div className="ml-1 mt-1 h-2.5 font-poppins text-[26px] font-bold uppercase text-navy-700 dark:text-white">
            Denku <span className="font-medium">MVP</span>
          </div>
        </div>
        <div className="mb-7 mt-[58px] h-px bg-gray-300 dark:bg-white/30" />
        {/* Nav item */}

        <ul className="mb-auto pt-1">
          <SidebarLinks routes={routes} />
        </ul>

        {/* Nav item end */}
      </div>
    </>
  );
}

export default SidebarAdapter;
