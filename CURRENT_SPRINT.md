...
| 8 | ~~Denku's own Inbox is unreadable~~ — **DONE 2026-09-03** | The owner's profile was MOVED to the `Denku` workspace (`286b7738`) with an UPDATE, not an INSERT. A second `profiles` row would have split the two resolvers — `getViewer()` matches on `id` first and would have kept authorizing against the old workspace, while `getActiveOrgId()` matches on `auth_user_id` ordered by `updated_at` and would have shown the new one (landmine #16). The old workspace `test2 llc` was empty, so it cost nothing; reversed by setting `org_id` back. |
...
| 10 | ~~A real voice call to Denku's own assistant~~ — **DONE 2026-09-03** | Two Turkish calls by the owner. The chain works: the tool was called, the org resolved, the honesty rules held (WhatsApp refused as not built). Four faults found and fixed the same day — `3600` read as "üç altı sıfır sıfır" digit by digit while 149/400/1200 were fine; `24/7` read as "yirmi dört yedi" when Turkish says 7/24; the same closing question after all four answers; and "what do you sell" answered without ever mentioning that voice and chat are separately-priced products or naming a single chat channel. ⚠️ **Not fixed, needs an ear:** the transcript renders the brand as "Denko"/"Dengo"/"DENQU" and SIP as "CIP", and one reply came back as the non-word "Denkoşu." — those are audio, not text, and the transcript is Vapi's own re-transcription, so it is unclear whether a caller actually hears them wrong. Someone has to listen to the recordings. |
| 11 | ~~Remove `VAPI_AGENT_ID` from Vercel~~ — **DONE 2026-09-03** | Deleted by the owner. It held `155b21ad…`, the old customer-shaped demo assistant. Nothing read it by then; the point of deleting was that a variable holding a stale assistant id is an invitation to someone later "fixing" the code to read it again — which would silently repoint the landing page back at the assistant this sprint replaced. |
