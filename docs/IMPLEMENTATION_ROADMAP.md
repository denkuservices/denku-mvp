...
### R-161 — Onboarding was English in Turkish, and the coverage test could not see it
...
runtime (`getDashboardDictionary("tr")` + `translateDashboardCopy`, not a reimplementation of
...
each screen was run through the real locale boundary. That is stronger than a source scan: it sees
text React composes at runtime, and it found three more classes of English no static pass could —
`"Step 4 of 6 · Plan"`, `"Back to What it answers"` and `"Pick any 2 of these"`, now runtime rules
guarded on a known step label. **Final state: nine screens, zero English text nodes.** The account
and its workspace were deleted and the deletion verified. Nothing was charged: the free branch
touches neither Stripe nor Vapi, and fetching markup fires no server action.

1859 tests pass; `tsc` clean on `src/`; `next build` green.
